    ...
    if ($action === 'status' && $id) {
        ...
        try {
            ...
            if (!empty($demande['email'])) {
                $emailSubject = $status === 'Acceptée' 
                    ? "Votre demande de document a été acceptée" 
                    : "Votre demande de document a été refusée";
                
                $emailMessage = $status === 'Acceptée'
                    ? "Bonjour {$demande['prenom']} {$demande['nom']},\n\n"
                      . "Votre demande de document (Type: {$demande['document_type']}, ID: #{$id}) a été acceptée.\n\n"
                      . "Le document vous sera envoyé prochainement.\n\n"
                      . "Cordialement,\nL'équipe administrative"
                    : "Bonjour {$demande['prenom']} {$demande['nom']},\n\n"
                      . "Votre demande de document (Type: {$demande['document_type']}, ID: #{$id}) a été refusée.\n\n"
                      . "Raison: " . ($justification ?? 'Aucune raison spécifiée') . "\n\n"
                      . "Cordialement,\nL'équipe administrative";
                
                // Appeler le service d'envoi d'email (simulé pour l'instant)
                // En production, utilisez un vrai service d'email
                try {
                    // Simuler l'envoi d'email
                    error_log("Email à envoyer à: {$demande['email']} | Sujet: $emailSubject");
                    
                    // Mettre à jour le statut email_sent
                    try {
                        $emailStmt = $pdo->prepare("UPDATE demande SET email_sent = TRUE, email_sent_at = NOW() WHERE id = ?");
            ...
            echo json_encode([
                ...
                'email_sent' => !empty($demande['email'])
            ]);
        } catch (PDOException $e) {
            http_response_code(500);
